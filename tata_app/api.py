...
#             # working model
#                 if get_lead==None:
#                     print("hello None lead, none opportunity, none customer")
...
def match_cl_no_supplier(cl_no1):
    ...
    b=frappe.db.sql(f"""select name from `tabSupplier` where phone_no='{i1}';""",as_list=True)
    # this is to check whether we have Lead or no? 
    # return b
    if len(b)>0:
        print("This is Equipment Process")
        o=frappe.db.sql(f"""select name from `tabItem` where supplier_number='{i1}';""",as_list=True)
        if o==[]:
            a=(b[0][0])
            return a
        ol=[]
...
def call_log():
    ...
    if len(non_match) > 0:
        ...
        for i in non_match:
            ...
            get_cust = match_cl_no_cus(cl_no1)
            get_lead = match_cl_no(cl_no1)
            get_supplier=match_cl_no_supplier(cl_no1)

            if get_supplier == None:
                if get_cust == None:
                    if get_lead == None:
                        print("hello None lead, none opportunity, none customer")
                        frappe.get_doc(({"doctype": "Call Logs", "id": i['id'], "call_id": i['call_id'], "service": i['service'], "date": i['date'], "call_duration": i['call_duration'], "department_name": i['department_name'], "agent_name": i['agent_name'], "agent_number": i[
                                   'agent_number'], "status": i['status'], "time": i['time'], "did_number": i['did_number'], "client_number": i['client_number'], "recording_url": i['recording_url'], "description": i['description'], "call_hint": i['call_hint']})).insert()
                    else:
                        print("This is true lead function")
                        # print(len(get_lead))
                        if len(get_lead) == 19:
                            print("this is the if-else for CRM-Only-Lead ")
                            frappe.get_doc(({"doctype": "Call Logs", "id": i['id'], "call_id": i['call_id'], "service": i['service'], "date": i['date'], "call_duration": i['call_duration'], "department_name": i['department_name'], "status": i['status'], "agent_name": i['agent_name'], "agent_number": i[
                                        'agent_number'], "status": i['status'], "time": i['time'], "did_number": i['did_number'], "client_number": i['client_number'], "recording_url": i['recording_url'], "lead": get_lead, "description": i['description'], "call_hint": i['call_hint']})).insert()
                            if i['status'] == "missed":
                                if i['call_hint'] == "clicktocall":
                                    content = call_miss_outgoing_icon(a, b)
                                else:
                            ...
                            print("this is the if-else for Lead-Opportunity ")
                            doc4 = frappe.get_doc(({"doctype": "Call Logs", "id": i['id'], "call_id": i['call_id'], "service": i['service'], "date": i['date'], "call_duration": i['call_duration'], "department_name": i['department_name'], "status": i['status'], "agent_name": i['agent_name'],
                                                "agent_number": i['agent_number'], "status": i['status'], "time": i['time'], "did_number": i['did_number'], "client_number": i['client_number'], "recording_url": i['recording_url'], "lead": get_lead[0], "description": i['description'], "call_hint": i['call_hint']}))
                            if i['status'] == "missed":
                                if i['call_hint'] == "clicktocall":
                                    content = call_miss_outgoing_icon(a, b)
                                else:
                            ...
                            for i in opo:
                                activity = frappe.get_doc(
                                    ...
                                    "content": content})
                                activity.insert()
                                doc4.append('opportunity_child',
                                            {'opportunity': i})
                            doc4.insert()
                else:            
                    if len(get_cust) == 3:
                        print("this is the if-else for Opportunity-Customer ")
                        doc3 = frappe.get_doc(({"doctype": "Call Logs", "id": i['id'], "call_id": i['call_id'], "service": i['service'], "date": i['date'], "call_duration": i['call_duration'], "department_name": i['department_name'], "status": i['status'], "agent_name": i['agent_name'],
                                            "agent_number": i['agent_number'], "status": i['status'], "time": i['time'], "did_number": i['did_number'], "client_number": i['client_number'], "recording_url": i['recording_url'], "customer": get_cust[0], "description": i['description'], "call_hint": i['call_hint']}))
                        if i['status'] == "missed":
                            if i['call_hint'] == "clicktocall":
                                content = call_miss_outgoing_icon(a, b)
                            else:
                        ...
                        print("this is the if-else for CRM-Only-Customer ")
                        frappe.get_doc(({"doctype": "Call Logs", "id": i['id'], "call_id": i['call_id'], "service": i['service'], "date": i['date'], "call_duration": i['call_duration'], "department_name": i['department_name'], "status": i['status'], "agent_name": i['agent_name'], "agent_number": i[
                                    'agent_number'], "status": i['status'], "time": i['time'], "did_number": i['did_number'], "client_number": i['client_number'], "recording_url": i['recording_url'], "customer": get_cust, "description": i['description'], "call_hint": i['call_hint']})).insert()
                        if i['status'] == "missed":
                            if i['call_hint'] == "clicktocall":
                                content = call_miss_outgoing_icon(a, b)
                            else:
                ...
                if len(get_supplier) == 3:
                    # working model     
                    print(get_supplier)
                    print("this is the if-else for Supplier and Equipment ")
                    doc3 = frappe.get_doc(({"doctype": "Call Logs", "id": i['id'], "call_id": i['call_id'], "service": i['service'], "date": i['date'], "call_duration": i['call_duration'], "department_name": i['department_name'], "status": i['status'], "agent_name": i['agent_name'],
                                          "agent_number": i['agent_number'], "status": i['status'], "time": i['time'], "did_number": i['did_number'], "client_number": i['client_number'], "recording_url": i['recording_url'], "supplier": get_supplier[0], "description": i['description'], "call_hint": i['call_hint']}))
                    if i['status'] == "missed":
                        if i['call_hint'] == "clicktocall":
                            content = call_miss_outgoing_icon(a, b)
                        else:
                    ...
                    activity = frappe.get_doc(
                        {"doctype": "Comment", "comment_type": "Label", "comment_email": "Administrator",
                         "reference_doctype": "Supplier", "reference_name": get_supplier[0],
                         "content": content})
                    activity.insert()
                    opo = get_supplier[2]
                    for i in opo:
                        activity = frappe.get_doc(
                            {"doctype": "Comment", "comment_type": "Label", "comment_email": "Administrator",
                             "reference_doctype": "Item", "reference_name": i,
                             "content": content})
                        activity.insert()
                        doc3.append('item_child', {'item': i})
                    doc3.insert()
                else:
                    print(get_supplier)
                    print("this is the if-else for Supplier ")
                    frappe.get_doc(({"doctype": "Call Logs", "id": i['id'], "call_id": i['call_id'], "service": i['service'], "date": i['date'], "call_duration": i['call_duration'], "department_name": i['department_name'], "status": i['status'], "agent_name": i['agent_name'], "agent_number": i[
                                   'agent_number'], "status": i['status'], "time": i['time'], "did_number": i['did_number'], "client_number": i['client_number'], "recording_url": i['recording_url'], "supplier": get_supplier, "description": i['description'], "call_hint": i['call_hint']})).insert()
                    if i['status'] == "missed":
                        if i['call_hint'] == "clicktocall":
                            content = call_miss_outgoing_icon(a, b)
                        else:
                        ...
                        activity = frappe.get_doc(
                        {"doctype": "Comment", "comment_type": "Label", "comment_email": "Administrator",
                         "reference_doctype": "Supplier", "reference_name": get_supplier,
                         "content": content})
                    activity.insert()
            frappe.db.commit()
            print("End Note")
